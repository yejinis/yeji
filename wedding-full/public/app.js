...
async function loadGallery(){
  ...
  try {
    ...
    grid.innerHTML = photos.map(p => `
      <button class="g-cell" data-full="${esc(p.full_url)}" data-name="${esc(p.uploader_name)}">
        <img src="${esc(p.thumb_url)}" alt="${esc(p.uploader_name)}님의 사진" loading="lazy">
      </button>`).join('');
    grid.querySelectorAll('.g-cell').forEach(c => c.addEventListener('click', () => openModal(c.dataset.full, c.dataset.name)));
  } catch (e) {
...
async function loadGuestbook(){
  ...
  try {
    ...
    list.innerHTML = entries.map(e => `
      <div class="gb-item" data-id="${e.id}">
        <div class="gb-top">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Q&A 아코디언
  document.querySelectorAll('.qa').forEach(q => q.addEventListener('click', () => q.classList.toggle('open')));

  // 계좌 토글
  const accBox = $('#acc-box'), accT = $('#acc-toggle');
  accT.addEventListener('click', () => { accBox.classList.toggle('open'); accT.textContent = accBox.classList.contains('open') ? '접기' : '계좌번호 보기'; });

  // D-day
  const target = new Date(2025, 11, 31); // ← (년, 월-1, 일) 로 수정하세요
  const diff = Math.ceil((target - new Date()) / 86400000);
  $('#dday').textContent = diff > 0 ? ('D-' + diff) : (diff === 0 ? 'D-DAY' : 'D+' + Math.abs(diff));
